...
for i = 1:length(T_vec)
    ...
    [~,~,~,~,Sd,~] = SDOF_Response_NL_1(T, zeta, ag_SLAC1_sc, dt, 0, 0, Cy_hi, 'Cy', 'linear');
    Sa_S1(i) = wn^2 * Sd / g;

    [~,~,~,~,Sd,~] = SDOF_Response_NL_1(T, zeta, ag_SLAC2_sc, dt, 0, 0, Cy_hi, 'Cy', 'linear');
    Sa_S2(i) = wn^2 * Sd / g;

    [~,~,~,~,Sd,~] = SDOF_Response_NL_1(T, zeta, ag_VA1_sc, dt, 0, 0, Cy_hi, 'Cy', 'linear');
    Sa_V1(i) = wn^2 * Sd / g;

    [~,~,~,~,Sd,~] = SDOF_Response_NL_1(T, zeta, ag_VA2_sc, dt, 0, 0, Cy_hi, 'Cy', 'linear');
    Sa_V2(i) = wn^2 * Sd / g;
end
