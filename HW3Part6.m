...
wn = 2*pi/T;
lat_strength = SDS / (R / Ie);
a = 60; % Site class D

%% Compute Sa values for 5%
[~,~,~,Sd5,~,~,~,~] = SDOF_Response(T, z5, ag_SLAC1_sc_dbe, dt, 0, 0);
Sa_S1_dbe = wn^2 * Sd5 / g;

[~,~,~,Sd5,~,~,~,~] = SDOF_Response(T, z5, ag_SLAC2_sc_dbe, dt, 0, 0);
Sa_S2_dbe = wn^2 * Sd5 / g;

[~,~,~,Sd5,~,~,~,~] = SDOF_Response(T, z5, ag_VA1_sc_dbe, dt, 0, 0);
Sa_V1_dbe = wn^2 * Sd5 / g;

[~,~,~,Sd5,~,~,~,~] = SDOF_Response(T, z5, ag_VA2_sc_dbe, dt, 0, 0);
Sa_V2_dbe = wn^2 * Sd5 / g;

% mu_strength per record
mu_S1_dbe = Sa_S1_dbe / lat_strength;
mu_S2_dbe = Sa_S2_dbe / lat_strength;
mu_V1_dbe = Sa_V1_dbe / lat_strength;
mu_V2_dbe = Sa_V2_dbe / lat_strength;
